...
from django.db.models import Max, Q  # Agregados para el buscador global
...
def _ensure_source_url(source: PriceDocSource):
    ...
    """
    if not source.url and source.doc_id and source.tipo != "web_html":
...
def _limpiar_historial_snapshots(source):
    """
    Mantiene únicamente los 2 últimos snapshots de la fuente.
    Borra los más antiguos para no saturar la BD.
    """
    snapshots_ids = list(
        PriceDocSnapshot.objects.filter(source=source)
        .order_by("-creado_en")
        .values_list("id", flat=True)
    )

    if len(snapshots_ids) > 2:
        ids_a_borrar = snapshots_ids[2:]
        PriceDocSnapshot.objects.filter(id__in=ids_a_borrar).delete()
        logger.info(f"Se limpiaron {len(ids_a_borrar)} snapshots antiguos de la fuente {source.nombre}.")


# =======================================================
# VISTA: BUSCADOR GLOBAL UNIFICADO
# =======================================================
...
def buscador_global_precios(request):
    query = request.GET.get("q", "").strip()
    
    # SOLUCIÓN: Atrapamos la lista pero descartamos automáticamente lo que no sea un número.
    # Si viene [''], lo convierte en [] (vacío) y evita el crasheo.
    fuentes_seleccionadas = [f for f in request.GET.getlist("fuente") if f.strip().isdigit()]
    
    fuentes_activas = PriceDocSource.objects.filter(activo=True)
    resultados_agrupados = {}
    total_encontrados = 0

    if query:
        snapshots_query = PriceDocSnapshot.objects.filter(source__activo=True)
        
        # Como sanitizamos la lista arriba, este if ahora funciona perfecto
        if fuentes_seleccionadas:
            snapshots_query = snapshots_query.filter(source_id__in=fuentes_seleccionadas)

        ultimos_snapshots_ids = snapshots_query.values("source_id").annotate(
            max_id=Max("id")
        ).values_list("max_id", flat=True)

        items_base = PriceDocItem.objects.filter(snapshot_id__in=ultimos_snapshots_ids)
        query_limpio = quitar_acentos(query)

        for item in items_base:
            producto_limpio = quitar_acentos(item.producto)
            art_limpio = quitar_acentos(item.art)
            
            if query_limpio in producto_limpio or query_limpio in art_limpio:
                nombre_fuente = item.snapshot.source.nombre
                
                if nombre_fuente not in resultados_agrupados:
                    resultados_agrupados[nombre_fuente] = []
                    
                resultados_agrupados[nombre_fuente].append(item)
                total_encontrados += 1

    # Convertimos los válidos a enteros para mantener tildados los checkboxes en el HTML
    fuentes_seleccionadas_int = [int(x) for x in fuentes_seleccionadas]

    return render(
        request,
        "integraciones/buscador_global.html",
        {
            "query": query,
            "fuentes_seleccionadas": fuentes_seleccionadas_int,
...
def price_source_sync(request, pk):
    ...
    try:
        ...
        # Bypass credentials if it's a web scraping source
        if source.tipo != "web_html":
            credentials = _get_google_credentials()
            
        cambios, snapshot = sync_price_source_by_id(source.id, credentials)

        # Lógica de máximo 2 snapshots
        _limpiar_historial_snapshots(source)
        ...
        if cambios > 0:
            messages.success(request, f"Fuente '{source.nombre}' sincronizada. Cambios: {cambios}.")
        else:
            hay_snapshots = PriceDocSnapshot.objects.filter(source=source).exists()
            if hay_snapshots:
                messages.info(request, f"Fuente '{source.nombre}' sincronizada sin cambios.")
            else:
                messages.info(request, f"Se creó el snapshot inicial de '{source.nombre}'.")

    except Exception as e:
        sa_email = getattr(credentials, "service_account_email", "desconocido") if credentials else "sin_credenciales"
        ...
        messages.error(request, f"Error al sincronizar '{source.nombre}': {e}")
...
def price_sources_sync_all(request):
    credentials = None
    try:
        # Obtenemos credenciales (el service se encargará de ignorarlas en los web_html)
        credentials = _get_google_credentials()
        ...
        messages.success(request, f"Procesadas {resultado['procesadas']} fuentes. Cambios: {resultado['total_cambios']}.")

    except Exception as e:
        messages.error(request, f"Error general al sincronizar: {e}")
...
def gestionar_cambios_doc_precios(request, source_id=None):
    ...
    if not source:
        messages.warning(request, "Todavía no hay fuentes de precios cargadas.")
        return redirect("price_sources_list")

    _ensure_source_url(source)

    if request.method == "POST":
        ids_aplicar = [int(x) for x in request.POST.getlist("candidato_id") if x.isdigit()]
        candidatos = PriceUpdateCandidate.objects.filter(
            source=source, pk__in=ids_aplicar, aplicado=False, producto__isnull=False,
        )
    ...
    try:
        ...
        # Limpiar snapshots viejos (Max 2)
        _limpiar_historial_snapshots(source)
        ...
        if n_cambios > 0:
            messages.info(request, f"Detectados {n_cambios} cambios nuevos.")
        else:
            hay_snapshots = PriceDocSnapshot.objects.filter(source=source).exists()
            if hay_snapshots:
                messages.info(request, f"Sincronización OK. Sin cambios nuevos.")
            else:
                messages.info(request, f"Snapshot inicial creado.")
...
def diagnostico_match_lista(request, source_id=None):
    ...
    if not snapshot:
        messages.info(request, f"La fuente '{source.nombre}' todavía no tiene snapshots.")
        return render(
            request,
...
def fetch_instagram_media():
    ...
    params = {"fields": "id,caption,media_type,media_url,permalink,thumbnail_url,timestamp", "access_token": access_token, "limit": limit}
    ...
    try:
        ...
        logger.error("Error Instagram: %s", e)
        return []

    media = []
    for item in data:
        media_type = item.get("media_type")
        image_url = item.get("thumbnail_url") or item.get("media_url") if media_type == "VIDEO" else item.get("media_url")
        if image_url:
            media.append({
                "id": item.get("id"), "image_url": image_url, "permalink": item.get("permalink"),
                "caption": item.get("caption", ""), "timestamp": item.get("timestamp", ""),
            })

    cache.set(INSTAGRAM_CACHE_KEY, media, getattr(settings, "INSTAGRAM_CACHE_SECONDS", 1800))
    return media